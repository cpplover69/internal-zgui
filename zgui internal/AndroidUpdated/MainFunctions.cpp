...
nom is very cool and made the zgui part thing bc zgui look cool yes
*/
...
#include <cstdlib>
#include <wchar.h>
#include "thing/zgui.cpp"
#include "thing/dependencies/utilities/render.cpp"
uint64_t BaseAddy = (uint64_t)GetModuleHandleA("svchost.exe"); //hehe
ImFont* m_pFont;
int hitbox;
float smooth = 1.0f;
...
namespace AndroidUpdated {
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		if (showMenu)
		{
			zgui::poll_input("Fortnite  "); //start input loop to forknife

			static bool example = false;
			static int example_int = 10;
			//start window yes
			if (zgui::begin_window("poopoo peepee woooo weeeeeee", { 500, 350 }, render::main_font, zgui::zgui_window_flags_none))
			{
				//menu items yes
				zgui::checkbox("nom very cool??", example);
				zgui::slider_int("how cool nom?", 0, 40, example_int);
				zgui::end_window();
			}
		}
	}
	...
	HRESULT __stdcall hk_present(IDXGISwapChain* pSwapChain, UINT sync_interval, UINT flags)
	{
		...
		// hehehe watermark
		DrawNormalText(m_pFont, TextFormat(xorstr("nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person nom cool person")), ImVec2(20, 20), 16.0f, ImColor(255, 255, 255, 255), false);
		...
		do {
			...
			//Get the GameInstance address with uworld + 0x160
			DWORD_PTR Gameinstance = ReadDWORD_PTR(AndroidNiceThings::Uworld, 0x160);
			//if it can't get the GameInstance address then break the loop
			if (!Gameinstance) break;

			//Get LocalPlayers address with uworld + 0x36
			DWORD_PTR LocalPlayers = ReadDWORD_PTR(Gameinstance, 0x36);
			...
			//Get the PlayerController address with LocalPlayer + 0x70
			AndroidNiceThings::PlayerController = ReadDWORD_PTR(AndroidNiceThings::Localplayer, 0x70);
			//if it can't get the PlayerController address then break the loop
			if (!AndroidNiceThings::PlayerController) break;

			//Get the LocalPawn Pointer address with PlayerController + 0x240
			auto LocalPawnInAuto = ReadPointer(AndroidNiceThings::PlayerController, 0x240);
			//if it can't get the LocalPawn address then break the loop
			if (!LocalPawnInAuto) break;

			//Get the LocalPawn address with PlayerController + 0x240
			AndroidNiceThings::LocalPawn = ReadDWORD_PTR(AndroidNiceThings::PlayerController, 0x240);
			//if it can't get the LocalPawn address then break the loop
			if (!AndroidNiceThings::LocalPawn) break;
			
			
			//Get the PlayerState address with LocalPawn + 0x230
			AndroidNiceThings::PlayerState = ReadDWORD_PTR(AndroidNiceThings::LocalPawn, 0x230);
			...
			if (AndroidNiceThings::LocalPawn != 0) {
				//Get the LocalPlayerID address with LocalPawn + 0x28
				AndroidNiceThings::localplayerID = ReadInt(AndroidNiceThings::LocalPawn, 0x28);
			}
			...
			for (auto li = 0UL; li < ReadDWORD(AndroidNiceThings::Uworld, 0x138 + sizeof(PVOID)); ++li) {
				...
				for (auto ai = 0UL; ai < ReadDWORD(level, 0x88 + sizeof(PVOID)); ++ai) {
					//Here we define the actors (the pawns)
					auto actors = ReadPointer(level, 0x88);
					if (!actors) break;
			...
			for (auto pawn : playerPawns) {
				...
				//Getting PlayerState address with pawn + 0x200
				auto state = ReadPointer(pawn, 0x200);
				
				if (!state) continue;

				//Getting EntityMesh address with pawn + 0x240
				DWORD_PTR player_mesh = ReadDWORD_PTR(pawn, 0x240); //mesh
				...
				Vector3 HeadPos = worker::GetBonePos(player_mesh, 0);

				//Get the Rootposition of LocalPawn (so the localplayer)
				Vector3 LocalPawnRootPos = ReadVector3(AndroidNiceThings::Rootcomp, 0x12C);


				//Doing worldtoscreen on HeadPosition
				Vector3 HeadPos_w2s = worker::W2S_IT(Vector3(HeadPos.x, HeadPos.y, HeadPos.z + 35), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
				...
				//Getting CornerWidth by doing the CornerHeight * 0.00
				float CornerWidth = CornerHeight * 0.00;
				...
				if (Configs.Skeletons) {
					...
					Vector3 Skel1 = worker::W2S_IT(worker::GetBonePos(player_mesh, 2), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
					Vector3 Skel2 = worker::W2S_IT(worker::GetBonePos(player_mesh, 63), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
					Vector3 Skel3 = worker::W2S_IT(worker::GetBonePos(player_mesh, 63), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
					Vector3 Skel4 = worker::W2S_IT(worker::GetBonePos(player_mesh, 68), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
					Vector3 Skel5 = worker::W2S_IT(worker::GetBonePos(player_mesh, 76), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
					Vector3 Skel6 = worker::W2S_IT(worker::GetBonePos(player_mesh, 69), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
					Vector3 Skel7 = worker::W2S_IT(worker::GetBonePos(player_mesh, 40), AndroidNiceThings::Localplayer, AndroidNiceThings::Rootcomp);
