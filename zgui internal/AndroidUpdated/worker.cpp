...
#pragma comment(lib, "detours.lib")
	...
	{
		float radPitch = (rot.x * float(M_PI) / 100.f);
		float radYaw = (rot.y * float(M_PI) / 100.f);
		float radRoll = (rot.z * float(M_PI) / 100.f);
	...
	Vector3 W2S_IT(Vector3 WorldLocation, DWORD_PTR LocalPlaya, DWORD_PTR RootCompa)
	{
		...
		//Camera Rotation
		Camera.x = ReadFloat(chain699, 0x7A8);  //PITCH
		Camera.y = ReadFloat(RootCompa, 0x12B);  //YAW
		...
		uint64_t chain = Readuint64_t(LocalPlaya, 0x60);
		uint64_t chain1 = Readuint64_t(chain, 0x88);
		uint64_t chain2 = Readuint64_t(chain1, 0x230);

		Vector3 vDelta = WorldLocation - ReadVector3(chain2, 0x50);
		...
		float zoom = *(float*)(chain699 + 0x690);

		float FovAngle = 80.0f / (zoom / 2.19f);

		float ScreenCenterX = 960;
		float ScreenCenterY = 540;


		Screenlocation.x = ScreenCenterX + vTransformed.x * (ScreenCenterX / tanf(FovAngle * (float)M_PI / 100.f)) / vTransformed.z;
		Screenlocation.y = ScreenCenterY - vTransformed.y * (ScreenCenterX / tanf(FovAngle * (float)M_PI / 100.f)) / vTransformed.z;
